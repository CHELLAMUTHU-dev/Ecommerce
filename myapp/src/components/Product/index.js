...
import {ProductContainer,LoaderContainer,ProductList,ProductHeading,FilterContainer,ClearButton,SearchContainer,FilterGroup,ErrorMessage,FilterHeading,Error,FilterItems,NoProductsContainer} from './styledComponents';
                        ...
                        <ProductContainer color={bgColor}>
                            ...
                            <FilterContainer>
                                    ...
                                    <FilterGroup color={color}>
                                        <p>Category</p>
                                        <select onChange={onChangeCategory} value={category} color={color}>
                                            {categoryList.map(eachItem => <option value={eachItem.value} >{eachItem.displayText}</option>)}
                                        </select>
                                    </FilterGroup>
                                    <FilterGroup color={color}>
                                    <p>Filter by</p>
                                    <select value={orderBy} onChange={onChangeOrderBy} color={color}>
                                        {orderByList.map(eachItem => <option value={eachItem.value}>{eachItem.displayText}</option>)}
                                    </select>
                                    </FilterGroup>
                                    <FilterGroup color={color}>
                                    <MdSort className="filter"/>
                                    <select value={order} onChange={onChangeOrder} color={color}>
                                        {orderList.map(eachItem => <option value={eachItem.value}>{eachItem.displayText}</option>)}
                                    </select>
                                    </FilterGroup>
                                    <FilterGroup color={color}>
                                    <p>Price</p>
                                    <select value={price} onChange={onChangePrice} color={color}>
                                        {priceList.map(eachItem => <option value={eachItem.displayText}>{eachItem.displayText}</option>)}
                                    </select>
                                    </FilterGroup>
