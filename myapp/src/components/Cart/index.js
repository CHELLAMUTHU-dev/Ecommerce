...
import {CartContainer,CartHeading,EmptyContainer,EmptyPara,CartItemContainer,CartItem,CartImg ,
    ...
    EmptyImg ,RemoveContainer,CartDetails,CartDetailsTitle ,CartDetailsBrand} from "./StyledComponents";
...
const Cart = () => {
    ...
    // const cartCount = cartList.reduce((total, item) => total + item.cartCount, 0);
    ...
    return (
        ...
        {cartList.length === 0 ? (
            ...
            {cartList.map((item) => (
                ...
                <Button style={{backgroundColor:'transparent', marginLeft:'50px' }} color={color} onClick={() => removeCartItem(item.id)}><CgCloseO/></Button>
                </CartItem>
            ))}
